...
async def process_Guinea_file(file: UploadFile = File(...)):
    try:
        ...
        direct_exp = df[df['EXPOSURE_TYPE'] == 'DIRECT']
        contingent_exp = df[df['EXPOSURE_TYPE'] == 'CONTINGENT']
        missed_repayments = df['UNPAID_AMOUNT_USD'].sum()
        sumof_all = df['CURRENT EXPOSURE (USD)'].sum()
        fcy_total = ccy['CURRENT EXPOSURE (USD)'].sum()
        fcy_direct = direct_exp[direct_exp['CURRENCY_TYPE'] == 'FCY']['CURRENT EXPOSURE (USD)'].sum()
        sumof_direct = direct_exp['CURRENT EXPOSURE (USD)'].sum()
        ...
        # Aggregated data for missed repayments
        aggregated_missed = df.groupby('CUSTOMER NAME')[['SECTOR ADJ', 'APPROVED AMOUNT (USD)', 'CURRENT EXPOSURE (USD)', 'UNPAID_AMOUNT_USD']].sum().reset_index()
        missed_repayments_data = aggregated_missed.sort_values(by='UNPAID_AMOUNT_USD', ascending=False).head(20)
